        ...
        <h3
          className={cn(
            ...
            hasUrgentStyling ? styles.headerClass : (
              item.isMuted ? "text-text-tertiary font-medium" : "text-text-secondary font-semibold"
            )
          )}
...
const VirtualItemRenderer = memo(({
  ...
  switch (item.type) {
    ...
    case "task": {
      ...
      const styles = urgencyStyles[dueDateGroupConfig[taskItem.sectionId as keyof TaskGroupByDate]?.urgency || "normal"]
    ...
    case "parent-task": {
      ...
      const styles = urgencyStyles[dueDateGroupConfig[parentItem.sectionId as keyof TaskGroupByDate]?.urgency || "normal"]
...
export const VirtualizedAllTasksView = ({
  ...
  const handleAddTaskForSection = useCallback((sectionKey: keyof TaskGroupByDate) => {
