...
export const MoreFiltersDropdown = ({
  ...
  const [localStatusIds, setLocalStatusIds] = useState<string[]>(selectedStatusIds)
  const [localRepeatType, setLocalRepeatType] = useState<RepeatFilterType>(repeatType)
  const [localHasTime, setLocalHasTime] = useState<HasTimeFilterType>(hasTime)

  // Sync local state when props change
  useMemo(() => {
    setLocalStatusIds(selectedStatusIds)
    setLocalRepeatType(repeatType)
    setLocalHasTime(hasTime)
  ...
  const handleApply = (): void => {
    onStatusChange(localStatusIds)
    onRepeatTypeChange(localRepeatType)
    onHasTimeChange(localHasTime)
    setIsOpen(false)
  }

  const handleClear = (): void => {
    setLocalStatusIds([])
    setLocalRepeatType("all")
    setLocalHasTime("all")
  }
        ...
        <div className="p-2 max-h-96 overflow-y-auto">
          ...
          {showStatusFilter && statuses.length > 0 && (
            <>
              ...
              {statuses.map((status) => {
                const isSelected = localStatusIds.includes(status.id)
          ...
          {repeatOptions.map((option) => {
            const isSelected = localRepeatType === option.value
            let taskCount = 0
            ...
            return (
              <button
                key={option.value}
                type="button"
                onClick={() => setLocalRepeatType(option.value)}
                className={cn(
                  "flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm transition-colors",
                  "hover:bg-accent focus:outline-none focus:bg-accent",
          ...
          {timeOptions.map((option) => {
            const isSelected = localHasTime === option.value
            let taskCount = 0
            ...
            return (
              <button
                key={option.value}
                type="button"
                onClick={() => setLocalHasTime(option.value)}
                className={cn(
                  "flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm transition-colors",
                  "hover:bg-accent focus:outline-none focus:bg-accent",
                  isSelected && "font-medium bg-accent"
