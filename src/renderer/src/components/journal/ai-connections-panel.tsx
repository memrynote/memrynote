...
import { Zap, BookOpen, FileText, ChevronRight, RefreshCw, Sparkles, AlertCircle } from 'lucide-react'
    ...
    return (
        ...
        >
            ...
            <div className="p-3">
                ...
                {/* Empty State */}
                {!isLoading && connections.length === 0 && !error && (
                    <EmptyState isNewUser={isNewUser} />
                )}
...
function PanelHeader({ count, isLoading, hasError, onRefresh }: PanelHeaderProps): React.JSX.Element {
    return (
...
function ConnectionItem({ connection, onClick }: ConnectionItemProps): React.JSX.Element {
    ...
    const label = connection.type === 'journal'
        ? connection.date
        : connection.title
