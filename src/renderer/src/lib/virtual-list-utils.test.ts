...
describe("virtual-list-utils", () => {
  ...
  describe("flattenTasksByDueDate (T117)", () => {
    ...
    it("should include section headers with correct urgency", () => {
      ...
      const overdueHeader = result.find(
        (item) => item.type === "section-header" && (item as SectionHeaderItem).sectionKey === "overdue"
      ) as SectionHeaderItem

      const todayHeader = result.find(
        (item) => item.type === "section-header" && (item as SectionHeaderItem).sectionKey === "today"
      ) as SectionHeaderItem
    ...
    it("should include add-task-button at end of each section", () => {
      ...
      const result = flattenTasksByDueDate([todayTask], [mockProject], emptyExpandedIds, [todayTask])
    ...
    it("should handle regular tasks as task type", () => {
      ...
      const result = flattenTasksByDueDate([regularTask], [mockProject], emptyExpandedIds, [regularTask])
    ...
    it("should set isOverdue flag for overdue section", () => {
      ...
      const result = flattenTasksByDueDate([overdueTask], [mockProject], emptyExpandedIds, [overdueTask])
  ...
  describe("flattenTasksByStatus (T119)", () => {
    ...
    it("should group tasks under correct status", () => {
      ...
      const inProgressTask = createMockTask({ id: "in-progress-task", statusId: "status-in-progress" })
      ...
      const todoHeaderIndex = result.findIndex(
        (item) => item.type === "status-header" && (item as StatusHeaderItem).status.id === "status-todo"
      )
